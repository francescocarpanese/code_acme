...
import numpy as np
from environments.MovingCoil0D_dm_control import Moving_Coil, Tasks

from dm_control.rl.control import Environment

# Test default init
def test_default_init():
    env = Environment(Moving_Coil.physics(),Tasks.Step())
    TimeStep = env.reset()
...
def test_N_timesteps():
    ...
    # Instantiate env
    environment = Environment(Moving_Coil.physics(),Tasks.Step())
...
def test_time_stepping_time_limit():
    # Instantiate env
    environment = Environment(Moving_Coil.physics(),Tasks.Step(t_step =1.), time_limit= 2.)
    TimeStep = environment.reset()
